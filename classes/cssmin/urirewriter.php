...
class Cssmin_Urirewriter
{
	...
	public static function rewrite($css, $current_dir, $doc_root = NULL, $symlinks = array())
	{
		...
		// Rewrite
		$css = preg_replace_callback('/@import\\s+([\'"])(.*?)[\'"]/',  array(self::$className, '_process_uri_cb'), $css);
		$css = preg_replace_callback('/url\\(\\s*([^\\)\\s]+)\\s*\\)/', array(self::$className, '_process_uri_cb'), $css);
	 ...
	 *
	 * <code>
	 * Cssmin_Urirewriter::rewrite_relative(
	 *       '../img/hello.gif'
	...
	public static function rewrite_relative($uri, $real_current_dir, $real_doc_root, $symlinks = array())
	{
		...
		$path = strtr($real_current_dir, '/', DIRECTORY_SEPARATOR).DIRECTORY_SEPARATOR.strtr($uri, '/', DIRECTORY_SEPARATOR);
	 ...
	 * if you extend Cssmin_Urirewriter
	 *
	 * @var string
	 */
	protected static $className = 'Cssmin_Urirewriter';
	...
	private static function _process_uri_cb($m)
	{
		...
		else
		{
			...
			$quote_char = ($m[1][0] === "'" || $m[1][0] === '"') ? $m[1][0] : '';
			$uri        = ($quote_char === '')                   ? $m[1]    : substr($m[1], 1, strlen($m[1]) - 2);
		}
		...
		return $is_import ? '@import '.$quote_char.$uri.$quote_char : 'url('.$quote_char.$uri.$quote_char.')';
	}
